import os    #os库是与操作系统相关的库，包含文件，目录，执行系统命令等方法
import sys   #python运行环境的库，包含各种环境操作，如sys.argv: 实现从程序外部向程序传递参数。sys.exit([arg]): 程序中间的退出，arg=0为正常退出。
import argparse  #添加一些参数
import random
import torch
...
import torch.utils.data    #包装数据
import torchvision.transforms as transforms   #data augmentation数据增强操作，图像缩放等。
...
blue = lambda x: '\033[94m' + x + '\033[0m'    #lambda匿名函数  blue(x),blue为函数名，x为参数传入,输出有颜色文本
BASE_DIR = os.path.dirname(os.path.abspath(__file__))   #abspath函数返回该脚本目录，dirname函数该py文件父目录
USE_CUDA = True
...
def weights_init_normal(m):       #参数权重正态分布初始化，torch.nn.init.normal_(tensor, mean=0, std=1)
...
if USE_CUDA:       
    print("Let's use", torch.cuda.device_count(), "GPUs!")   #检查有几个GPU
    point_netG = torch.nn.DataParallel(point_netG)    #使用nn.DataParallel来包装我们的模型(数据)，使之并行操作
    point_netD = torch.nn.DataParallel(point_netD)
    point_netG.to(device)                  #将我们的模型放在device上
    point_netG.apply(weights_init_normal)   #应用初始参数
...
if opt.manualSeed is None:   #设置随机种子，当其数值固定时，随机产生的参数是固定的
...
# setup optimizer
optimizerD = torch.optim.Adam(point_netD.parameters(), lr=0.0001,betas=(0.9, 0.999),eps=1e-05,weight_decay=opt.weight_decay)  #权重衰减
optimizerG = torch.optim.Adam(point_netG.parameters(), lr=0.0001,betas=(0.9, 0.999),eps=1e-05 ,weight_decay=opt.weight_decay)
schedulerD = torch.optim.lr_scheduler.StepLR(optimizerD, step_size=40, gamma=0.2)  #随着epoch增大动态调整学习率
